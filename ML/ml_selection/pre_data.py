...
# def pca_merge_correlated_columns(data, positive_threshold=0.9, negative_threshold=-0.9):
# def pca_merge_correlated_columns(data, positive_threshold=0.6, negative_threshold=-0.6):
#     """
#     상관계수가 높은 컬럼들을 PCA로 병합하고 기존 컬럼을 제거하는 함수.

#     Parameters:
#     - data: 변환할 데이터프레임
#     - positive_threshold: 양의 상관계수를 기준으로 그룹화할 임계값
#     - negative_threshold: 음의 상관계수를 기준으로 그룹화할 임계값
    
#     Returns:
#     - data_pca_merged: PCA로 변환된 컬럼들만 포함한 데이터프레임
#     - positive_correlated_groups: 양의 상관관계로 그룹화된 피처 리스트
#     - negative_correlated_groups: 음의 상관관계로 그룹화된 피처 리스트
#     """
